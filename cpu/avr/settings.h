...
/**
 * Gets setting associated with specified key and index.
 * 
 * @param key Key that identifies entry
 * @param index Number of entry of this key, 0 for single entry
 * @param value pointer to storage location for read value
 * @param value_size pointer to size entry. If value of referenced size is zero, 
 *   the size of the requested entry is written to it.
 *   If it is non-zero it specifies the maximum number of bytes to read.
 * @return status feedback
 */
extern settings_status_t settings_get(settings_key_t key, uint8_t index, unsigned char* value, size_t* value_size);
/**
 * Adds setting associated with specified key.
 * 
 * @param key Key that identifies entry
 * @param value pointer to storage location of value to write
 * @param value_size size of the setting
 * @return status feedback
 */
extern settings_status_t settings_add(settings_key_t key, const unsigned char* value, size_t value_size);
/**
 * Checks setting associated with specified key.
 * 
 * @param key Key that identifies entry
 * @param index Number of entry of this key, 0 for single entry
