...
void discovery_scheduler_pattern2_func(void * ptr);

void discovery_scheduler_pattern2_init() {
	ctimer_set(&dst, DTN_DISCO_TIMESLOT_LENGTH * CLOCK_SECOND, discovery_scheduler_pattern2_func, NULL);
}
...
void discovery_scheduler_pattern2_set_schedule_index(uint8_t index) {
...
const struct discovery_scheduler_driver discovery_scheduler_pattern2 = {
		.init = discovery_scheduler_pattern2_init,
		.set_schedule_index = discovery_scheduler_pattern2_set_schedule_index,
};
