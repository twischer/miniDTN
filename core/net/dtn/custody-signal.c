...
void copy_timestamp_to_signal (struct bundle_t *bundle , custody_signal_t *return_signal) {
	...
	sdnv_decode(bundle->block + bundle->offset_tab[TIME_STAMP][OFFSET], bundle->offset_tab[TIME_STAMP][STATE], return_signal->bundle_creation_timestamp);
	sdnv_decode(bundle->block + bundle->offset_tab[TIME_STAMP_SEQ_NR][OFFSET], bundel->offset_tab[TIME_STAMP_SEQ_NR][STATE], return_signal->bundle_creation_timestamp_seq);

}

void copy_eid_to_signal(bundle_t *bundle, custody_signal_t *return_signal) {
	
	sdnv_decode(bundle->block + bundle->offset_tab[SRC_NODE][OFFSET], bundle->offset_tab[SRC_NODE][STATE], return_signal->src_node);

	sdnv_decode(bundle->block + bundle->offset_tab[SRC_APP][OFFSET], bundle->offset_tab[SRC_APP][STATE], return_signal->src_app);
